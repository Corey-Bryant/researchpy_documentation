...
Input
-----
**ANOVA(formula, data = None, sum_of_squares = 3, conf_level = 0.95, display_summary = True, table_decimals = None)**
  ...
  * **sum_of_squares** : The type of sum of squares which is desired, the default is Type 3. Valid options are ``1``/``"I"`` (Sequential), ``2``/``"II"`` (Hierarchical/Partially sequential), and ``3``/``"III"`` (Marginal/Orthogonal).
...
The ``Anova`` class is also available under the alias ``ANOVA``.
...
^^^^^^^^^^^^^
  ...
  * **results(include_test_stat_p = True, include_effect_sizes = True, factor_effects = True, return_type = "Dataframe", na_rep = '', pretty_format = True, table_decimals = None)**

      * **include_test_stat_p** : Whether to include p-values for the test statistics in the results. The default is ``True``.
      * **include_effect_sizes** : Whether to include effect sizes in the results. The default is ``True``.
      * **factor_effects** : Whether to include the per-factor effects in the ANOVA table. The default is ``True``.
  ...
  -results- returns a ``ModelResults`` dataclass with the fields ``model_name``, ``fit_statistics``, ``model_table``, ``coefficients``, and ``details``. The dataclass supports both attribute access (e.g., ``result.model_table``) and tuple unpacking::

      name, fit_stats, anova_table, coefs, details = m.results()

  * **regression_table(return_type = "Dataframe")**
...
.. code:: python
  ...
  m = ANOVA("systolic ~ C(drug) + C(disease) + C(drug):C(disease)", data = systolic, sum_of_squares = 3)
   ...
   name, desc, table, coefs, details = m.results()
